...
class ChildChecklist {
  constructor(creator_account, checklist_id) {
    this.creator_account = creator_account, this.checklist_id = checklist_id;
  }
}
class MasterChecklist {
  constructor(creator_account, master_checklist_id) {
    this.creator_account = creator_account;
    this.master_checklist_id = master_checklist_id;
  }
}
let Checklists = (_dec = NearBindgen({}), _dec2 = call({}), _dec3 = call({}), _dec4 = call({}), _dec(_class = (_class2 = class Checklists {
  constructor() {
    this.assignedChecklist = new LookupMap("a");
...
function unassignChecklist() {
  const _state = Checklists._getState();
  if (!_state && Checklists._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  const _contract = Checklists._create();
  if (_state) {
    Checklists._reconstruct(_contract, _state);
  }
  const _args = Checklists._getArgs();
  const _result = _contract.unassignChecklist(_args);
  Checklists._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(Checklists._serialize(_result, true));
}
function createChecklist() {
  ...
  const _args = Checklists._getArgs();
  const _result = _contract.createChecklist(_args);
  Checklists._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(Checklists._serialize(_result, true));
}
function assignChecklist() {
  ...
  const _args = Checklists._getArgs();
  const _result = _contract.assignChecklist(_args);
  Checklists._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(Checklists._serialize(_result, true));
}

export { Checklists, assignChecklist, createChecklist, unassignChecklist };
